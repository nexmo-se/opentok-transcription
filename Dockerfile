...
RUN apt-get -y install cmake gcc g++ extra-cmake-modules build-essential && export CC=/usr/bin/clang && export CXX=/usr/bin/clang++ && mkdir src/build && cd src/build && CC=clang CXX=clang++ cmake ..
RUN make
RUN cd ..
